...
class GameEnvironment:
    def __init__(self):
        ...
        if self.monitor_region is None or self.monitor_region.get('width', 0) == 0:
            logging.error(f"Could not find or get valid dimensions for game window '{config.WINDOW_TITLE_SUBSTRING}'. Using fallback or first monitor as final.")
            self.monitor_region = config.FALLBACK_GAME_REGION if config.FALLBACK_GAME_REGION else self.sct.monitors[1]
            if self.monitor_region is None or self.monitor_region.get('width',0) == 0:
                 logging.critical("FATAL: No valid monitor region could be established. Exiting possibility.")
